...
- **Kleuren via tokens.** Hardcode nooit een kleur voor accent/selectie/focus — gebruik `var(--accent)` (en bestaande `--accent-bg/-border/-text`, `--sel-bg`, `--danger-*`). Alleen échte data-kleuren (event-/template-kleur, `EVENT_COLORS`, de heatmap-schaal) blijven letterlijke hex. Het accent is **thema-bewust**: gebruik voor tekst/iconen óp het accent altijd `var(--accent-contrast)`, nooit een harde `#fff` (in dark is dat donker).
- **Drie font-tiers.** `var(--ff-display)` (Schibsted Grotesk) voor de grootste titels (paginatitel, datum-/weekkop), `var(--ff-head)` (Outfit) voor kaart-/sectietitels + nav-labels, `var(--ff-body)` (DM Sans) voor lopende tekst.
